...
async function callMuseLLM(baseName:string,dur:number,transcript:string|null, contextPackage:any={}):Promise<any|null>{
  ...
  const tr=transcript?`TRANSCRIPT (wajib pakai untuk caption/hook/seo, JANGAN ngarang): """${(transcript.length>3000 ? transcript.slice(0,3000) : transcript)}"""`:`Tanpa transcript (file testsrc), buat generic.`;
...
async function executeRealFFmpegPipeline(jobId: string, inputPath: string, sourceName: string, opts: ClipOptions) {
  ...
  try {
    ...
    // Clamp ke durasi asli, tanpa drop agresif (prompt stabil, caption tetap LLM)
    {
      ...
      for(const c of sorted){
        ...
        if(!isFinite(s)||!isFinite(e)){ job.logs.push(`[${new Date().toLocaleTimeString('id-ID')}] Clip drop: bukan angka`); continue; }
        s=Math.min(Math.max(0,s),Math.max(0,probeDurForNarrative-5)); e=Math.min(Math.max(s+5,e),probeDurForNarrative);
        c.start_time=Math.round(s); c.end_time=Math.round(e); valid.push(c);
      }
      activeClips=valid;
    }
