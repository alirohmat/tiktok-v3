...
function normalizeNiche(raw:any){ const tag=(typeof raw?.tag==='string'&&raw.tag.trim())?raw.tag.trim().toLowerCase().slice(0,32): (typeof raw?.niche_tag==='string'&&raw.niche_tag.trim()?raw.niche_tag.trim().toLowerCase().slice(0,32):'unknown'); const tier=normalizeNicheTier(raw?.tier??raw?.niche_profit_tier); let sc=Number(raw?.score??raw?.niche_score); if(!Number.isFinite(sc)) sc=0; sc=Math.round(Math.max(0,Math.min(100,sc))); const adv=(typeof raw?.advisory==='string'&&raw.advisory.trim())?raw.advisory.trim().slice(0,200): (typeof raw?.niche_advisory==='string'&&raw.niche_advisory.trim()?raw.niche_advisory.trim().slice(0,200): (tier==='high'?'High profit niche — upload 06:30 & 19:00 WIB': tier==='medium'?'Medium niche — upload 19:00 WIB':'Low niche — upload 19:30 WIB, cek riset')) ; return {tag,tier,score:sc,advisory:adv}; }
